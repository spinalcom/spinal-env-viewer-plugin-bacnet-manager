<template>
	...
	>
		...
		<md-dialog-content class="content">
			...
			<div class="loading" v-else-if="pageSelected === PAGES.loading">
				<md-progress-spinner md-mode="indeterminate"></md-progress-spinner>
			</div>

			<div class="loading" v-else-if="pageSelected === PAGES.success">
				<md-icon class="md-size-5x">done</md-icon>
			</div>

			<div class="loading" v-else-if="pageSelected === PAGES.error">
...
<script>
	...
	export default {
		...
		methods: {
			...
			disabled() {
				return (
					!this.deviceSelected || this.pageSelected === this.PAGES.success
				);
			},
			...
			updateDevices() {
				...
				if (this.profilSelected) {
					let val = this.profils.find(
						(el) => el.id === this.profilSelected
					);
